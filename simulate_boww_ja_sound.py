#!/usr/bin/env python3
"""
BoWWSimulator 日本語版 サウンド付き（ツーショット対応）
マイク入力 → Silero VAD → WAV保存 → Parakeet2HA日本語版（サウンド付き）に送信

ツーショット動作:
1. 「アレクサ」→ ウェイクワード検出 → 待機状態（wake.wav再生）
2. 「ライトつけて」→ インテント処理（success.wav再生）
"""
import os
import sys
import time
...
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - [BoWW_JA_SND] - %(message)s'
)
logger = logging.getLogger("BoWW_JA_Sound")


class BoWWSimulatorJASound:
    """日本語版 BoWW シミュレーター（サウンド付きサーバー用）"""
    ...
    def __init__(self):
        ...
        # 出力先（サウンド版用）
        self.spool_dir = "/tmp/boww_spool_ja_sound"
    ...
    def send_to_parakeet(self, json_path: str):
        """Parakeet2HA日本語版（サウンド付き）にIPC送信"""
        logger.info(f"🔗 Parakeet2HA日本語版（サウンド付き）に送信中...")
        try:
            ...
            logger.error("   main_ja_sound.py が起動していますか？")
...
if __name__ == "__main__":
    simulator = BoWWSimulatorJASound()
    simulator.run()
