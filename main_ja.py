...
from core.intent_parser_ja import IntentParserJA, Intent

logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - [JA] - %(message)s'
)
logger = logging.getLogger("Parakeet_JA")
...
class ParakeetJA:
    """日本語版 Parakeet ASR + インテントパーサー（ツーショット対応）"""
...
def transcribe_file(model: ParakeetJA, audio_path: str, output_json: bool = True):
    """単発ファイル認識（IPCなし）"""
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Parakeet2HA 日本語版")
...
if __name__ == "__main__":
    try:
        ...
        print("\n👋 終了します...")
        socket_path = "/tmp/parakeet2ha_ja.sock"
